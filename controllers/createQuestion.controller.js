...
export const generatePDF = async (req, res) => {
  try {
    ...
    // ✅ PDF setup
    const filename = `output_${Date.now()}_${Math.floor(
      Math.random() * 10000
    )}.pdf`;
    ...
    if (!fs.existsSync(fontPath)) {
      return res
        .status(500)
        .send("⚠️ Kalpurush.ttf ফন্ট পাওয়া যায়নি।");
    }
    ...
    for (const [idx, q] of questions.entries()) {
      ...
      if (q.stimulusImage) {
        try {
          const imagePath = path.join(__dirname, "..", q.stimulusImage);
          ...
          if (fs.existsSync(imagePath)) {
            ...
            doc.text("[⚠️ ছবি পাওয়া যায়নি]", { align: "center" });
          }
        } catch (err) {
          console.error("Image error:", err);
          doc.text("[⚠️ ছবি লোড ব্যর্থ]", { align: "center" });
        }
      }
    ...
    stream.on("finish", async () => {
      try {
        ...
        console.error(err);
        res.status(500).send("⚠️ PDF ডাউনলোডে সমস্যা হয়েছে");
      }
    });
  } catch (err) {
    console.error(err);
